...
/// 定期调用 [UpdateService.checkAll] 检查所有模拟器的版本更新，并在发现新版本时
...
class BackgroundTask {
  ...
  /// 已存在同名任务时使用 [ExistingWorkPolicy.replace] 覆盖。
...
/// 才能访问 `rootBundle` 与平台通道（如 path_provider）。
...
void callbackDispatcher() {
  Workmanager().executeTask((task, inputData) async {
    ...
    try {
      ...
      final result = await updateService.checkAll(emulators);
